                                ...
                                <div class="card-body billing-form">
                                        ...
                                        if (!empty($edit_order) && is_array($edit_order)) {
                                            if (!empty($edit_order['invoice_date'])) {
                                                ...
                                                $pi_due_date_val = !empty($edit_order['due_date']) ? substr($edit_order['due_date'], 0, 10) : $pi_order_date_val;
                                            } else {
                                                $pi_due_date_val = !empty($edit_order['due_date']) ? substr($edit_order['due_date'], 0, 10) : $pi_due_date_val;
                                            }
...
<script>
    ...
    if (!empty($edit_order_id) && $edit_order_id > 0 && !empty($edit_order) && is_array($edit_order)) {
        ...
        $base_url = (isset($SiteUrl) ? rtrim($SiteUrl, '/') . '/' : '') . 'admin/';
        foreach ($embed_items as &$emb_it) {
            if (!empty($emb_it['images'])) {
                ...
                if ($dec && !empty($dec['images']) && is_array($dec['images'])) {
                    $urls = array_map(function($p) use ($base_url) { return $base_url . ltrim($p, '/'); }, $dec['images']);
                    $primary = isset($dec['primary']) ? $dec['primary'] : $dec['images'][0];
                    $primary_url = $base_url . ltrim($primary, '/');
    ...
    /** Same characteristic or same synced barcode line — stops duplicate rows when scanning purchase siblings. */
    function isProductLineInProductListPi(barcode, characteristicId, purchaseInvoiceItemId) {
    ...
    function fetchProductByBarcodeAndAdd(barcode) {
        ...
        fetch('ajax/get-product-by-barcode.php?barcode=' + encodeURIComponent(trimmed) + '&expand_invoice_siblings=1&include_purchase_invoice_sibling_barcodes=1&allow_non_stock_barcode=1')
            ...
            .then(function(data) {
                if (data.success && data.products && data.products.length) {
                    ...
                    for (var pi = 0; pi < data.products.length; pi++) {
                        ...
                        if (isProductLineInProductListPi(rbc, rcid, pr.purchase_invoice_item_id)) continue;
                ...
                if (data.success && data.product) {
                    ...
                    if (isProductLineInProductListPi(resolvedBc, resCid, data.product.purchase_invoice_item_id)) {
                    ...
                    function addSiblingSequentialPi(idx, list) {
                        ...
                        fetch('ajax/get-product-by-barcode.php?barcode=' + encodeURIComponent(sb) + '&expand_invoice_siblings=1&include_purchase_invoice_sibling_barcodes=1&allow_non_stock_barcode=1')
                            .then(parseBarcodeAjaxJsonPi)
                            .then(function(d2) {
                                if (d2.success && d2.products && d2.products.length) {
                                    for (var j = 0; j < d2.products.length; j++) {
                                        ...
                                        if (!isProductLineInProductListPi(b2, c2, p2.purchase_invoice_item_id)) {
                                    ...
                                    if (!isProductLineInProductListPi(b2, c2, d2.product.purchase_invoice_item_id)) {
        ...
        .then(data => {
            if (data.status === 'success' || data.success === true) {
                ...
                if (data.customer_id && document.getElementById('customerId')) {
                    ...
                    // Programmatic .value does not fire change; Add Item state listens on #customerId (auragold-gst-lines.js)
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function applyStockJournalToModalRow(row, sj) {
        ...
        var inputMap = [
            ...
            ['purchase_amount', 'purchase-amount'], ['sale_amount', 'sale-amount'], ['sale_amount_with', 'sale-amount-with'],
