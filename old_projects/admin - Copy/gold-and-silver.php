...
/**
 * DB stores paths like uploads/stock_journal/file.jpg; files are on disk under admin/uploads/...
 */
function gas_upload_public_rel(string $rel): string {
    $r = ltrim($rel, '/');
    if ($r === '') {
        return '';
    }
...
/**
 * Browser URL for a stored path such as uploads/stock_journal/file.jpg.
 * Files live under admin/uploads/...; URLs must include /admin/ before uploads/.
 */
function gas_public_url_for_stored_path(?string $path, $SiteUrl): string {
    ...
    if (strpos($path, '/') === 0) {
        if (preg_match('#^/uploads/#', $path)) {
            $under = gas_upload_public_rel(ltrim($path, '/'));
    ...
    $rel = ltrim($path, '/');
    $underAdmin = gas_upload_public_rel($rel);
    $base = isset($SiteUrl) ? rtrim((string) $SiteUrl, '/') : '';
    if ($base !== '') {
        return $base . '/' . $underAdmin;
    }
    $appRoot = gas_app_web_root_path_from_script();
    if ($appRoot !== '') {
        return $appRoot . '/' . $underAdmin;
    }
    return '/' . $underAdmin;
}
...
// Net on hand per barcode: receipt-side movements minus outward rows (same idea as Stock Availability Wt /
// gold-silver-analysis.php). Summing current_* across tbl_stock rows double-counts because each voucher is its own row.
...
<style>
    ...
    .gas-count { font-size: 0.82rem; color: #64748b; font-weight: 600; }
</style>
<body>
                    ...
                    <div class="gas-wrap">
                        <div class="gas-head">
                            <div>
                                ...
                                <p class="gas-count mb-0">Barcode-level on-hand (inward movement totals minus outward rows), same net idea as Stock Availability — Gold / Silver metals only.</p>
                            </div>
                            <div class="gas-tabs">
