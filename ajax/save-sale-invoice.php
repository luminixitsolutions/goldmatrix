...
try {
    ...
    // For new invoices, if invoice_no already exists in sale or POS table, bump until unique (same series pattern)
    if (!$is_update) {
    ...
    if ($is_update) {
        ...
        // Check if invoice_no is being changed and if it conflicts with another sale or POS invoice
        if ($invoice_no !== $current_invoice_no) {
            $existing_invoice = getRecord("SELECT id FROM tbl_sale_invoices WHERE invoice_no = '$invoice_no' AND id != $invoice_id");
            if (!$existing_invoice) {
                $existing_invoice = getRecord("SELECT id FROM tbl_pos_sale_invoices WHERE invoice_no = '$invoice_no'");
            }
            if ($existing_invoice) {
